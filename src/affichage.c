...
void generer_lignes_carte(int carte, char lignes[5][20]){
    const char *couleur;
    if(carte >= 13){
        couleur = YELLOW; //bonus card
    } 
    else{
        couleur = BLUE; //normal card
    }

    //top of the cards
    sprintf(lignes[0], "%s--------%s", couleur, COLOR_RESET); 

    //content of the card, according to the card
    switch(carte){ 
        case 13:
...
void afficher_plateau(Joueur *j, Pioche *p){
    ...
    int progression = (p->prochain_indice * 40)/p->taille; //pick's scale
